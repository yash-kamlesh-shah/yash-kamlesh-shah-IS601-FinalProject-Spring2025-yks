...
@pytest.mark.asyncio
async def test_delete_user_access_denied(async_client, admin_user, user_token):
    headers = {"Authorization": f"Bearer {user_token}"}
...
@pytest.mark.asyncio
async def test_delete_user_access_allowed(async_client, admin_user, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    delete_response = await async_client.delete(f"/users/{admin_user.id}", headers=headers)
